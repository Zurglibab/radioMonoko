    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                alert(t("contact.alertErrorForm"));
            }
        } catch (error) {
            console.error("Erreur réseau :", error);
            alert(t("contact.alertErrorNetwork"));
        } finally {
            setIsLoading(false);
        }
    };
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                        <div className="flex flex-col md:flex-row gap-6">
                            <div className="flex-1 flex flex-col gap-2">
                                <label htmlFor="name" className={`text-sm font-semibold uppercase tracking-wider ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'}`}>{t("contact.name")}</label>
                                <input
                                    type="text"
                                    id="name"
                                    name="name"
                            ...
                            <div className="flex-1 flex flex-col gap-2">
                                <label htmlFor="email" className={`text-sm font-semibold uppercase tracking-wider ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'}`}>{t("contact.email")}</label>
                                <input
                                    type="email"
                                    id="email"
                                    name="email"
                        ...
                        <div className="flex flex-col gap-2">
                            <label htmlFor="subject" className={`text-sm font-semibold uppercase tracking-wider ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'}`}>{t("contact.subject")}</label>
                            <input
                                type="text"
                                id="subject"
                                name="subject"
                        ...
                        <div className="flex flex-col gap-2">
                            <label htmlFor="message" className={`text-sm font-semibold uppercase tracking-wider ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'}`}>{t("contact.message")}</label>
                            <textarea
                                id="message"
                                name="message"
