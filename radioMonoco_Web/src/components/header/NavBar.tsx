...
const NavBar = () => {
    ...
    const IconCircleStyle = "flex items-center justify-center w-9 h-9 md:w-10 md:h-10 rounded-full transition-all duration-300 cursor-pointer active:scale-95 hover:bg-white/10 group";
    ...
    return (
        <>
                    ...
                    {isConnected && (
                        <div
                            className={`
                                ...
                                ${isSearchOpen
                                ? 'bg-white/10 px-3 w-40 md:w-72 border border-white/10 ring-1 ring-white/5 shadow-lg'
                        ...
                        >
                            ...
                            <input
                                ...
                                className={`
                                    ml-10 bg-transparent border-none outline-none text-white text-[10px] md:text-xs font-black 
                    ...
                    {isConnected ? (
                        <>
                            <div className="relative" ref={notifRef}>
                                <button onClick={() => { setIsNotifOpen(!isNotifOpen); setIsProfileOpen(false); }} className={`${IconCircleStyle} ${isNotifOpen ? 'bg-white/20' : ''}`}>
                                    <HiOutlineBell className={`text-lg md:text-xl transition-colors cursor-pointer ${isNotifOpen ? 'text-rose-500' : 'text-white'}`} />
                            ...
                            <div className="relative" ref={dropdownRef}>
                                <button onClick={() => { setIsProfileOpen(!isProfileOpen); setIsNotifOpen(false); }} className={`${IconCircleStyle} ${isProfileOpen ? 'bg-white/20' : ''}`}>
                                    <HiOutlineUser className={`text-lg md:text-xl transition-colors cursor-pointer ${isProfileOpen ? 'text-rose-500' : 'text-white'}`} />
                                </button>
                                {isProfileOpen && (
                                    <div className="absolute top-12 right-0 w-56 md:w-64 bg-[#111] border border-white/10 rounded-2xl shadow-2xl py-3 animate-in fade-in zoom-in-95 duration-150 z-[60]">
                                        ...
                                        <button onClick={() => { setIsSettingsOpen(true); setIsProfileOpen(false); }} className="w-full flex items-center gap-3 px-4 py-2 text-sm text-neutral-300 hover:bg-white/5 transition-colors text-left cursor-pointer outline-none">
                                            <HiOutlineCog className="text-lg" /> Paramètres
                                        </button>
                                        <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-2 text-sm text-rose-500 hover:bg-rose-500/10 transition-colors text-left cursor-pointer outline-none">
                        ...
                        <div className="flex items-center gap-2 md:gap-4">
                            {/* Connexion - Style Ghost */}
                            <Link
                                to="/login"
                                className="group flex items-center justify-center h-9 md:h-10 px-4 rounded-full border border-white/10 hover:border-white/30 transition-all duration-300 active:scale-95"
                            >
                            ...
                            {/* Inscription - Style Action (Solid) */}
                            <Link
                                to="/register"
                                className="group flex items-center justify-center h-9 md:h-10 px-4 md:px-6 rounded-full bg-rose-600 hover:bg-rose-500 transition-all duration-300 active:scale-95 shadow-lg shadow-rose-600/20"
                            >
            ...
            <div className={`fixed inset-0 z-[100] flex items-center transition-all duration-400 md:duration-700 ${isMenuOpen ? "visible opacity-100" : "invisible opacity-0"}`}>
                <div className={`absolute inset-0 bg-[#050505]/98 backdrop-blur-3xl transition-opacity duration-400 md:duration-700 cursor-pointer ${isMenuOpen ? "opacity-100" : "opacity-0"}`} onClick={() => setIsMenuOpen(false)} />
